...
let reserved =
  [ "true"; "false"; "if"; "then"; "else"; "while"; "do"; "and"; "or" ]
...
and if_stmt input =
  ...
  >>= fun pred ->
    ...
    >> (* { *)
    stmts
    >>= fun els -> token "}" >> return (IfElse (pred, thn, els)) )
    input
...
and while_stmt input =
  ...
  >>= fun guard ->
    ...
    >> (* { *)
    stmts
    >>= fun body -> token "}" >> return (While (guard, body)) )
    input
...
let rec eval prog env =
  ...
  | IfElse (pred, thn, els) ->
      if eval_bexp pred env then eval thn env else eval els env
  | While (guard, body) ->
      let rec loop () =
...
and eval_aexp aexp env =
  match aexp with
  | PlusExp (l, r) ->
      let l' = eval_aexp l env and r' = eval_aexp r env in
      l' + r'
  | SubExp (l, r) ->
      let l' = eval_aexp l env and r' = eval_aexp r env in
      l' - r'
...
and eval_bexp bexp env =
  ...
  | GTExp (l, r) ->
      ...
      l' > r'
  | AndExp (l, r) ->
      let l' = eval_bexp l env and r' = eval_bexp r env in
      l' && r'
  | OrExp (l, r) ->
      let l' = eval_bexp l env and r' = eval_bexp r env in
      l' || r'
